  ...
  const productCards = products.slice(0,60).map(p=>{
    ...
    return `
      <div class="pos-card ${outOfStock?'out-of-stock':''}" data-pos-add="${p.id}" title="${outOfStock?'Out of stock at this branch':'Click to add to cart'}">
        ...
        <div class="pos-card-meta">${p.category}</div>
    ...
    <div class="pos-shell">
      <div style="display:flex; flex-direction:column;">
        <div class="pos-search-bar">
          <input class="pos-search-input" id="posSearchInput" placeholder="Search by name, category, or scan barcode..." value="${POS_SEARCH}" autofocus>
        </div>
        <div class="pos-grid">
